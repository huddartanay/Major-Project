...
| **A stream-health check detects `imu_dropout` where the conformal gate does not** | **Established** | E21: `health` 30/30 at 0.000 clean false positives over 30 clean runs, median latency 5 ticks, against OD-8's 0/30. Same seeds, faults and injection mode as R3c | **Yes** |
| **The sustained-fault blind spot is architectural, not statistical** | **Established** | E21: the evidence is present at L1 and cheap to read; the gate does not consult it. Detecting a dropout by stream health is near-definitional, and that is the point | **Yes** |
| **`health` and OD-8 are complementary; neither is sufficient** | **Established** | E21: `health` catches `imu_dropout` (OD-8 0.00), OD-8 catches `lateral_noise` (`health` 0.00). Union covers 4 of 6 faults against OD-8's 3 | **Yes** |
| **`speed_bias` and `speed_stuck` are undetected by every monitor tested** | **Established** | E21: OD-8, `health`, `innovation` and `trust` all fail both, in transient and sustained modes | **Yes** |
| The innovation-sequence detector is a usable baseline at its committed threshold | **Rejected** | E21: 0.00 on five of six faults, 0.20 on `lateral_noise` at 2,154-tick median latency. Inert at its ceiling of 3.0 | **No** |
| The Trust Index is usable as a fault detector at its committed floor | **Rejected** | E21: fires on 23 of 30 **clean** runs (0.767), far outside the 0.10 false-positive bar | **No** |
| ~~A clean run produces no blocking verdicts in the evaluation window~~ | **WITHDRAWN** | 12-seed sweep, 9 Sep 2026: clean runs produce 0-3 blocking verdicts in ticks 200-399, median 1, and 9 of 12 seeds produce at least one | **No** |
| ~~Losing a channel disturbs the downstream verdict trace~~ | **Seed-dependent, not a property** | 12-seed sweep: the faulted arm produced 0-84 blocking verdicts and **3 of 12 seeds produced none**. Holds on 9 of 12; a single-seed test cannot carry it | **Only as a tendency, with n stated** |
| **The integrity counter saturates under `imu_dropout`; the OOD counter does not** | **Established** | 12-seed sweep: integrity reached exactly 40 -- its configured ceiling -- on 12 of 12 seeds, while OOD ranged 1-22 against its own ceiling of 100 | **Yes** |
