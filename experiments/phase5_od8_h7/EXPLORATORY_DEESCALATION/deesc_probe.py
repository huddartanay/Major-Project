...
from benchmarks.e21_baseline import (
    CHANNEL_SIGMAS, FAULT_FIRST, POLICY_PATH, _injector_sustained,
    _sensing_sustained, _severity,
)
...
def run(policy, fault, seed, ticks):
    ...
    inj = (_injector_sustained(fault, mag, seed) if active
...
def analyse(rows, active):
    ...
    for i in range(1, len(rows)):
        ...
        if RANK[st] < RANK[prev] and (not active or t >= FAULT_FIRST):
            events.append({"tick": t, "from": prev, "to": st, "ood": ood,
                           "integrity": integ, "l1_unhealthy": unhealthy})
    ...
    return {"peak": [k for k, v in RANK.items() if v == peak][0],
            "final": rows[-1][1], "n_deescalations": len(events),
            "first_deesc": events[:3], "nominal_frac_after_onset": round(nominal_frac, 3),
            "veto_frac_after_onset": round(veto_frac, 4),
            "l1_unhealthy_frac_after_onset": round(unhealthy_frac, 3)}


if __name__ == "__main__":
    ticks = int(sys.argv[1]); seeds = int(sys.argv[2]); faults = sys.argv[3].split(",")
    out = Path(sys.argv[4])
