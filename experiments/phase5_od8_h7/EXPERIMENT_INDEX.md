...
| **STEP1-MECHANISM** | Why does the gate go silent under sustained faults? | **Complete on dev** | σ, departure, quantile all pinned at 1.00x clean; alarm-rate suppression is a 38x tail effect on a distribution held constant while true tracking error grows 1.5-7x | **H-none per pre-committed rule → target ITSC (see observations.md for a candidate C2-side hypothesis)** |
