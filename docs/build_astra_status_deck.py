...
def slide(prs, title=None, kicker=None, dark=False):
    ...
    if title:
        if kicker:
            p(
                tb(s, Inches(0.7), Inches(0.40), Inches(11.9), Inches(0.26)),
                kicker.upper(),
                10,
                TEAL,
                bold=True,
        ...
        p(
            tb(s, Inches(0.7), Inches(0.66), Inches(11.9), Inches(0.62)),
            title,
            32,
            INK if not dark else WHITE,
            bold=True,
            first=True,
            space=0,
            font=HEAD,
...
def foot(s, text, col=FAINT):
    p(
        tb(s, Inches(0.7), Inches(6.96), Inches(11.9), Inches(0.3)),
        text,
        8.5,
        col,
        first=True,
...
def build(out: Path) -> None:
    ...
    p(
        tf,
        "Monitoring in a Layered Governance Stack",
        21,
        RGBColor(0xD6, 0xE3, 0xEE),
        space=14,
        font=HEAD,
    )
    p(
        tf,
        "Where does sensor-fault evidence stop being usable — and can that "
        "be turned into a working monitor?",
        13,
        RGBColor(0x9F, 0xB8, 0xCC),
        space=0,
    )
    p(
        tb(s, Inches(0.85), Inches(6.35), Inches(7.4), Inches(0.7)),
        "Sushanth C.  ·  Tarun Gowda V  ·  T Tilak Reddy\n"
        "Guide: Dr. Chaitra R.  ·  BMS College of Engineering  ·  1 September 2026",
        10.5,
        RGBColor(0x82, 0x9C, 0xB4),
        first=True,
        space=0,
    )
    ...
    p(
        tf,
        "A sensor fault does not stay a sensor fault.",
        15,
        INK,
        bold=True,
        first=True,
    ...
    p(
        tf,
        "Conventional fault detection answers a binary question — “fault or no fault?” "
    ...
    p(
        tf,
        "ASTRA is a 9-layer runtime governance stack for a learned controller, "
    ...
    card(
        ...
        [
            (
                ...
                "being distinguishable from healthy behaviour?",
                INK,
                True,
            )
        ],
    ...
    foot(
        s,
        "Nine layers: L1 sensing · L2 dual-rate UKF · L3 trust · L4 learned proposer (Core-A) · "
    ...
    p(
        tb(s, Inches(0.7), Inches(1.5), Inches(11.9), Inches(0.3)),
        "Four commitments that make the numbers mean something",
        12.5,
        BODY,
        first=True,
        space=0,
    ...
    items = [
        (
            ...
            "D_s(f) = AUC(faulted, matched-clean), folded to [0.5, 1.0]. "
            "0.5 is chance-level separation, never “50 % accuracy”. Reported per stage as a profile, "
            "not collapsed to one number.",
        ),
        (
            "Delivered-signal integrity",
            "A result is rejected unless the faulted arm is shown to differ "
        ...
        (
            "Pre-registration",
            "Stage, faults, thresholds, tests and falsification criteria are written "
            "and frozen before execution. Deviations are reported as deviations.",
        ),
        (
            "The run is the unit",
            "Ticks inside a run are autocorrelated and are never treated as "
    ...
    foot(
        s,
        "Metric appropriateness is audited per fault type: dispersion faults use a rolling-standard-deviation "
    ...
    p(
        ...
        "3 / 3 Core-B gates return a verdict on every one of 300 ticks.",
        11.5,
        BODY,
        first=True,
        space=0,
    ...
    for code, name, col in cells:
        ...
        x += bw + gap
    for lo, wd, lab in [
        (Inches(0.7), Inches(4.72), "SHARED   L1–L3"),
        ...
        p(
            tb(s, lo, Inches(3.14), wd, Inches(0.25)),
            lab,
            9.5,
            BODY,
            first=True,
    ...
    table(
        ...
        [
            ...
            [
                "L2b / L3",
                "estimated lateral position; trust index",
                "post-estimation state and trust",
            ],
            [
                "L6",
                "conformal non-conformity score",
                "statistical gate evidence — and its threshold",
            ],
    ...
    foot(
        s,
        "The one-way Core-A → Core-B trust boundary is enforced by architectural invariants: "
    ...
    p(
        ...
        "status is tracked honestly, item by item.",
        11.5,
        BODY,
        first=True,
        space=0,
    ...
    rows = [
        ...
        (
            "2",
            "Fault-specific observability profiles — four distinct behaviours measured",
            "ESTABLISHED",
        ),
        ...
        (
            "5",
            "Diagnosing why a conformal monitor fails, at the level of its score process",
            "ESTABLISHED",
        ),
    ...
    for n, txt, st in rows:
        ...
        dot(
            ...
            y + Inches(0.11),
            n,
            col=DEEP if st in ("ESTABLISHED", "PARTIAL") else GREY,
        )
        p(
            tb(s, Inches(1.42), y + Inches(0.17), Inches(8.9), Inches(0.3)),
            txt,
            11.5,
            INK,
            first=True,
    ...
    card(
        ...
        [
            (
                ...
                "literature review and completed experiments. No priority claim is made.",
                INK,
                True,
            )
        ],
    ...
    table(
        ...
        [
            [
                "Dimension",
                "Conventional FDI / residual monitoring",
                "Conformal & runtime verification",
                "ASTRA's position",
            ],
            [
                "Question asked",
                "Is a fault present now?",
                "Is this observation conformal to calibration?",
                "Where does the evidence stop being usable?",
            ],
            [
                "Output",
                "binary flag per detector",
                "prediction set or alarm",
                "a per-stage evidence profile D_f",
            ],
            [
                "Placement of monitors",
                "engineering convention or intuition",
                "wherever the score is defined",
                "proposed to follow measured observability",
            ],
            [
                "Treatment of the estimator",
                "residuals treated as the signal",
                "estimator usually outside scope",
                "estimator treated as a transformation that can remove evidence",
            ],
            [
                "Failure reporting",
                "missed detection / false alarm rates",
                "coverage guarantee",
            ...
            [
                ...
                "—",
                "—",
                "a measured separation of discriminability, calibration validity and operational stability",
            ],
        ],
        widths=[2.0, 3.2, 3.2, 3.5],
        fs=10,
        hfs=9.5,
        rh=Inches(0.62),
    )
    ...
    for code, name, st, pct in stages:
        col = CHIP[st]
        p(
            tb(s, Inches(0.7), y + Inches(0.03), Inches(1.15), Inches(0.28)),
            code,
            11,
            INK,
            bold=True,
        ...
        p(
            tb(s, Inches(1.85), y + Inches(0.04), Inches(1.7), Inches(0.28)),
            name,
            10,
            BODY,
            first=True,
    ...
    p(
        tb(s, Inches(3.66), Inches(3.42), Inches(3.0), Inches(0.3)),
        "◀  we are here",
        11,
        BAD,
        bold=True,
    ...
    card(
        ...
        [
            (
                "5 of 11 stages. All internal validation. 4,290 closed-loop runs, "
    ...
    foot(
        s,
        "Stages are gated: a stage is not entered because it exists in the plan. External datasets "
    ...
    table(
        ...
        [
            ...
            [
                "E17",
                "Stage-wise fault observability",
                "90 profiles · 2,160 runs",
                "COMPLETE",
                "Observability is heterogeneous; 1 of 6 faults well-posed",
            ],
            [
                "E17-Position",
                "Does absorption survive correct injection?",
                "720 runs",
                "WITHDRAWN",
                "Absorbed in 0 of 12 cells — original result was artefact",
            ],
            [
                "E18",
                "Can OD-8 be validly calibrated?",
                "72,000 ticks · 1,260 runs",
                "PARTIAL",
                "D_s does not predict detection; window defect found",
            ],
            [
                "E18-R1",
                "Does run-local calibration recover P3?",
                "60 runs",
                "REJECTED",
                "P3 9/30; mechanism removed, estimator too noisy",
            ],
            [
                "E18-R2",
                "Does matched-window calibration work?",
                "reused scores",
                "PARTIAL",
                "P1 13/30 — best of three schemes, still short",
            ],
        ],
        widths=[1.5, 3.1, 2.0, 1.4, 4.2],
        fs=10,
        hfs=9.5,
        rh=Inches(0.58),
    ...
    foot(
        s,
        "Every invalidated record is retained and flagged, never deleted — an audit that erases its "
    ...
    left = [
        ...
        (
            ...
            "causally: estimator sees 0 with redundancy on, 3.18 with it off.",
            "ESTABLISHED",
        ),
    ]
    right = [
        ...
        (
            ...
            "of the score, not of the threshold.",
            "ESTABLISHED",
        ),
    ]
    ...
    foot(
        s,
        "All results are [M-syn] — synthetic plant, three policies, one severity per fault where "
    ...
    p(
        tb(s, Inches(0.7), Inches(1.48), Inches(11.9), Inches(0.3)),
        "The threshold is in the right place. The score has no room around it.",
        13,
        INK,
        bold=True,
        first=True,
    ...
    for i, (v, l, c) in enumerate(
        [
            ("0.0097", "score standard deviation (P1)", DEEP),
    ...
    card(
        ...
        [
            (
                ...
                "score's own baseline wanders by 0.013–0.028 between runs.",
                INK,
                True,
            ),
            (
                "The precision required is 3–6× finer than the system's own reproducibility. "
                "Signal-to-noise at the decision boundary is below 1.",
                BODY,
            ),
        ],
        TINT,
    ...
    table(
        ...
        [
            [
                "Policy",
                "Overdispersion vs binomial",
                "Effective ticks (of 200)",
                "Alarm autocorrelation",
                "Mechanism",
            ],
            ["P1", "4.2×", "11", "+0.359", "alarms cluster within a run"],
            ["P3", "9.2×", "2", "≈ 0", "baseline varies between runs"],
        ],
        widths=[1.1, 2.6, 2.4, 2.4, 3.4],
        fs=10.5,
    )
    foot(
        s,
        "An ideal monitor with independent ticks would place 29.2 of 30 runs in band. Three calibration "
    ...
    p(
        ...
        "before publication. Each one withdrew a claim.",
        11.5,
        BODY,
        first=True,
        space=0,
    ...
    withdrawn = [
        ...
        (
            "H-regime operating-regime covariate",
            "Simpson's paradox: pooled −0.341, within-policy +0.836",
        ),
        (
            "OD-8 provides operational monitoring",
            "no policy is calibrated on the evaluation window",
        ),
    ]
    y = Inches(2.0)
    for h, why in withdrawn:
        ...
        p(
            tb(s, Inches(2.35), y + Inches(0.07), Inches(5.6), Inches(0.46)),
            h,
            11,
            INK,
            bold=True,
        ...
        p(
            tb(s, Inches(2.35), y + Inches(0.30), Inches(5.6), Inches(0.26)),
            why,
            9.5,
            BODY,
            first=True,
    ...
    card(
        ...
        [
            ...
            ("Alarm suppression is not better detection.", BODY),
        ],
        TINT,
        DEEP,
        hs=11.5,
    ...
    card(
        ...
        [
            (
                ...
                "requires delivered-signal verification before any sweep is believed.",
                INK,
                True,
            )
        ],
    ...
    foot(
        s,
        "Full status of every claim, with permitted wording, is maintained in CLAIM_LEDGER.md and "
    ...
    p(
        tb(s, Inches(0.7), Inches(1.42), Inches(11.9), Inches(0.32)),
        "The immediate question is narrow, cheap, and decisive.",
        13,
        RGBColor(0x9F, 0xB8, 0xCC),
        first=True,
        space=0,
    ...
    steps = [
        (
            ...
            "A pure compute change. Separates “not enough samples” from “wrong monitor”.",
            "NEXT",
        ),
        (
            "E19 / H7",
            ...
            "derived from D_s and is pre-registered — even though E18 suggests it will fail.",
            "BLOCKED",
        ),
        (
            "E20",
            "Adversarial / lying sensor",
            "One channel lies while two stay honest, through the per-channel injection path already "
            "built. Not started; no detection claim is made.",
            "NOT STARTED",
        ),
        (
            "External",
            "comma2k19 → highD → CARLA",
            "Real driving data, then traffic regimes, then closed loop. Gated behind a working "
            "internal measurement chain.",
            "NOT STARTED",
        ),
    ]
    y = Inches(1.95)
    for code, name, body, st in steps:
        ...
        p(
            tb(s, Inches(1.0), y + Inches(0.17), Inches(1.6), Inches(0.3)),
            code,
            13,
            RGBColor(0x6E, 0xC8, 0xE8),
            bold=True,
        ...
        p(
            tb(s, Inches(1.0), y + Inches(0.52), Inches(2.1), Inches(0.32)),
            name,
            10.5,
        ...
        p(
            tb(s, Inches(3.3), y + Inches(0.22), Inches(7.6), Inches(0.7)),
            body,
            10.5,
            RGBColor(0xC8, 0xD8, 0xE6),
            first=True,
            space=0,
        )
