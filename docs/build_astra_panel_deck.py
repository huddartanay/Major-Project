...
def slide(prs, title=None, kicker=None, dark=False):
    ...
    if title:
        if kicker:
            p(
                tb(s, Inches(0.6), Inches(0.38), Inches(12.1), Inches(0.24)),
                kicker.upper(),
                9.5,
                ACCENT,
                bold=True,
        ...
        p(
            tb(s, Inches(0.6), Inches(0.64), Inches(12.1), Inches(0.6)),
            title,
            26,
...
def table(
    s, x, y, w, rows, widths=None, fs=10, hfs=10, rh=Inches(0.28), hh=Inches(0.30), colors=None
):
...
def foot(s, text):
    p(
        tb(s, Inches(0.6), Inches(7.02), Inches(12.1), Inches(0.28)),
        text,
        8.5,
        FAINT,
        first=True,
...
def build(out: Path) -> None:
    ...
    p(
        tb(s, Inches(0.9), Inches(1.35), Inches(11.5), Inches(0.4)),
        "MAJOR PROJECT PANEL REVIEW  ·  DEPARTMENT OF COMPUTER SCIENCE & ENGINEERING",
        11,
        RGBColor(0x7F, 0xC5, 0xD1),
        bold=True,
        first=True,
        space=0,
    )
    ...
    p(
        tf,
        "Autonomous Safety-Trust Runtime Architecture",
        22,
        RGBColor(0xC6, 0xDD, 0xE3),
        space=6,
        font=HEAD,
    )
    p(
        tf,
        "A layered runtime governance stack for learned controllers, and an "
        "experimental study of when its safety monitors can and cannot be trusted",
        13,
        RGBColor(0x94, 0xB8, 0xC3),
        italic=True,
        space=0,
    )
    ...
    p(
        tf,
        "A nine-layer runtime safety architecture for a vehicle driven by a learned "
    ...
    p(
        tf,
        "Autonomous cyber-physical systems increasingly rely on learned controllers whose "
    ...
    foot(
        s,
        "All results are from a synthetic driving plant. External real-world validation is not "
    ...
    p(
        tf,
        "A learned controller in a vehicle depends on sensor input. Sensors fail — bias, "
        ...
        "a safe fallback before harm occurs.",
        11.5,
        BODY,
        space=8,
    )
    p(
        tf,
        "The safety of the whole system therefore depends on the monitors being reliable. "
    ...
    p(
        tf,
        "Analytical verification of learned controllers is not feasible at scale. The "
    ...
    p(
        tf,
        "Learned controllers are already deployed at scale in production ADAS. Public data "
    ...
    p(
        tf,
        "We wanted a testbed where a fault could be traced through every layer of a "
    ...
    p(tf, "WHY EXISTING APPROACHES ARE INSUFFICIENT", 10, ACCENT, bold=True, first=True, space=8)
    rows = [
        (
            "Simplex (2001)",
            "prescribes fallback controller and switching logic; does not "
            "prescribe how the decision logic itself is judged reliable",
        ),
        (
            "Innovation-based FDI",
            "detects at one stage; not routinely evaluated against a "
            "learned controller's downstream behaviour",
        ),
        (
            "Conformal prediction",
            "provides coverage under exchangeability; live systems "
            "routinely violate exchangeability without warning",
        ),
        (
            "Digital-twin / model-based",
            "monitors residuals; assumes model and system stay "
            "aligned, an assumption that itself needs checking",
        ),
        (
            "Redundant sensor fusion",
            "voting outvotes a liar and discards the evidence; "
    ...
    p(
        tf,
        "Six objectives, each traceable to a specific experimental deliverable in the "
        "project record.",
        11,
        MUTE,
        first=True,
    ...
    objs = [
        (
            "1",
            "Design a layered runtime governance architecture",
            ACCENT,
        ...
        (
            "2",
            "Build a controlled fault-injection testbed",
            ACCENT,
        ...
        (
            "3",
            "Characterise stage-wise fault observability",
            ACCENT,
        ...
        (
            "4",
            "Establish calibration and detection behaviour of a conformal safety monitor",
            WARN,
        ...
        (
            "5",
            "Separate statistical discriminability from operational detectability",
            WARN,
        ...
        (
            "6",
            "Propose an architectural response — a self-trust layer",
            MUTE,
    ...
    for n, title, col, body in objs:
        ...
        p(
            tb(s, Inches(0.6), y + Inches(0.22), Inches(0.65), Inches(0.4)),
            n,
            22,
            WHITE,
            bold=True,
    ...
    foot(
        s,
        "Objectives 1-5 are complete or substantially complete; objective 6 is designed with "
    ...
    table(
        ...
        [
            ...
            [
                "Simplex (Sha 2001)",
                "Complex controller + safety controller + switching logic",
    ...
    p(
        tf,
        "Existing runtime safety architectures assume their monitors work. There is no "
    ...
    foot(
        s,
        "References available in accompanying literature-review document; systematic review "
    ...
    p(
        tf,
        "A layered governance stack around a learned controller, plus a self-assessment "
    ...
    p(
        tf,
        "The self-assessment mechanism is what makes an operator able to say, at runtime, "
        "how much to trust the current output of a specific monitor. That is a capability "
        "no existing architecture in this space provides.",
        10.5,
        BODY,
        space=0,
    )
    ...
    for i, (code, name, col) in enumerate(
        [
            ("L1", "sensing", ACCENT_DK),
            ("L2", "UKF estimator", ACCENT_DK),
            ("L3", "trust", ACCENT_DK),
            ("L4", "learned proposer  (CORE-A)", MUTE),
            ("L5", "PINN twin", ACCENT),
            ("L6", "conformal gate", ACCENT),
            ("L7", "shield", ACCENT),
            ("L8", "fail-safe", ACCENT),
    ...
    p(
        tb(s, Inches(10.02), Inches(3.7), Inches(0.5), Inches(0.4)),
        "→",
        22,
        MUTE,
        first=True,
    ...
    p(
        tf,
        "measures each monitor's dynamic range, drift, and phase state",
        9,
        RGBColor(0xE4, 0xD9, 0xEF),
        space=8,
        align=PP_ALIGN.CENTER,
    )
    p(
        tf,
        "attaches a trust score to every verdict",
        9,
        RGBColor(0xE4, 0xD9, 0xEF),
        space=0,
        align=PP_ALIGN.CENTER,
    ...
    foot(
        s,
        "The stack (L1-L9) is implemented and characterised. The self-trust plane is a design "
    ...
    p(
        tb(s, Inches(0.6), Inches(1.35), Inches(12.1), Inches(0.3)),
        "Sensor input → estimation → trust → learned proposal → twin prediction → statistical "
        "and deterministic gates → fail-safe → arbitration → actuation.",
        10.5,
        MUTE,
        first=True,
        space=0,
    ...
    p(
        tb(s, Inches(0.6), Inches(1.92), Inches(2.7), Inches(0.28)),
        "SENSORS  IMU · GPS · LIDAR · CAN · fused frame",
        9.5,
        WHITE,
        bold=True,
        first=True,
    ...
    p(
        tb(s, Inches(0.7), Inches(2.5), Inches(4.4), Inches(0.28)),
        "SHARED  (perception & estimation)",
        10,
        INK,
        bold=True,
        first=True,
    ...
    for i, (c, txt) in enumerate(
        [
            ("L1", "Sensor bus, staleness rule, per-channel health"),
        ...
        p(
            tb(s, Inches(0.7), yy + Inches(0.05), Inches(0.6), Inches(0.24)),
            c,
            10,
            WHITE,
            bold=True,
        ...
        p(
            tb(s, Inches(1.35), yy + Inches(0.08), Inches(3.8), Inches(0.28)),
            txt,
            9.5,
            BODY,
            first=True,
    ...
    p(
        tb(s, Inches(5.45), Inches(2.5), Inches(1.6), Inches(0.28)),
        "CORE-A",
        10,
        INK,
        bold=True,
    ...
    p(
        tb(s, Inches(5.45), Inches(2.95), Inches(0.6), Inches(0.24)),
        "L4",
        10,
        WHITE,
        bold=True,
    ...
    p(
        tb(s, Inches(5.45), Inches(3.35), Inches(1.6), Inches(1.25)),
        "Learned proposer\n(PPO with PID-Lagrangian CMDP)\n\nWrites π_prop to Core-B; may not "
        "read Core-B artefacts.",
        8.5,
        BODY,
        first=True,
        space=0,
    ...
    p(
        tb(s, Inches(7.4), Inches(2.5), Inches(5.2), Inches(0.28)),
        "CORE-B  (safety gates, verdict on every tick)",
        10,
        INK,
        bold=True,
        first=True,
    ...
    for i, (c, txt) in enumerate(
        ...
        p(
            tb(s, Inches(7.4), yy + Inches(0.05), Inches(0.6), Inches(0.24)),
            c,
            10,
            WHITE,
            bold=True,
        ...
        p(
            tb(s, Inches(8.05), yy + Inches(0.08), Inches(4.55), Inches(0.28)),
            txt,
            9.5,
            BODY,
            first=True,
    ...
    p(
        tb(s, Inches(0.6), Inches(4.92), Inches(12.1), Inches(0.28)),
    ...
    p(
        tf,
        "OBSERVABILITY PLANE  (offline; read-only taps into every layer above)",
        10,
        INPT,
        bold=True,
        first=True,
    ...
    p(
        tf,
        "Fault-evidence profiler  ·  stability analyzer (σ_between, n_eff, autocorrelation)  ·  "
    ...
    p(
        tf,
        "The observability plane never writes to Core-A. It is a monitor for the monitors, "
    ...
    foot(
        s,
        "Diagram based on `src/astra/` module layout; 9/9 layers domain-mapped; 3/3 Core-B "
    ...
    p(
        tf,
        "Every 50 ms (20 Hz fast rate), the following sequence executes for one control tick.",
        10.5,
        MUTE,
        first=True,
        space=0,
    ...
    steps = [
        (
            "1",
            "Sensor acquisition",
            "IMU/GPS/LIDAR/CAN publish into a shared bus. Timestamps "
            "and per-channel health are recorded. Redundant sensing draws independent noise per "
            "channel from a disjoint seed.",
        ),
        (
            "2",
            "L1 fusion & health check",
            "Frame assembled; L1 evaluates staleness against a "
            "50 ms budget and flags per-modality health.",
        ),
        (
            "3",
            "L2 estimation (dual-rate UKF)",
            "Predict + update on the fused frame. Records "
            "the innovation vector, its Mahalanobis distance, and the covariance-normalised residual.",
        ),
        (
            "4",
            "L3 trust classification",
            "Context classifier (Mondrian scheme) assigns the tick "
            "to a context class (URBAN_CLEAR, HIGHWAY_CLEAR, DEGRADED_SENSOR); trust index emitted.",
        ),
        (
            "5",
            "L4 (Core-A) proposes π_prop",
            "Learned PPO policy consumes the fast state and "
            "trust and emits a command proposal. This is the only write from Core-A into Core-B.",
        ),
        (
            "6",
            "L5 twin prediction",
            "PINN twin produces the command that modelled physics "
            "expects, conditioned on the L3 context class.",
        ),
        (
            "7",
            "L6 statistical gate",
            "Compute non-conformity score = ‖π_prop − π_twin‖ / √P̂_a. "
            "Compare against the calibrated conformal quantile → PASS / VETO.",
        ),
        (
            "8",
            "L7 shield + L7b physical envelope",
            "Deterministic bounds on speed, lateral "
            "acceleration, jerk. VETO if any bound is exceeded.",
        ),
        (
            "9",
            "L8 fail-safe FSM",
            "Selects between issued command, degraded command, or halt "
            "posture based on gate verdicts and posture history.",
        ),
        (
            "10",
            "L9 arbitration → actuation",
            "Emits the safe command to the actuator; writes "
    ...
    for i, (n, ttl, body) in enumerate(steps):
        ...
        p(
            tb(s, x, y + Inches(0.27), Inches(0.5), Inches(0.4)),
            n,
            15,
            WHITE,
            bold=True,
    ...
    foot(
        s,
        "Fault injection, when active, is applied at step 1/2 (sensor level) or step 5-6 "
    ...
    stacks = [
        (
            ...
            [
                (
                    "Python 3.12",
                    "single language across pipeline, training and analysis; "
        ...
        (
            ...
            [
                (
                    "statistics.NormalDist + custom impl",
                    "Wilcoxon, Holm-Bonferroni, BCa "
        ...
        (
            ...
            [
                ...
                (
                    ...
                    "enforces Core-A → Core-B one-way boundary as architectural invariant",
                ),
            ],
        ),
        (
            ...
            [
                (
                    "python-pptx (isolated venv)",
                    "presentation generation without touching the measurement lockfile",
                ),
    ...
    for i, (title, col, items) in enumerate(stacks):
        ...
        p(
            tb(s, x + Inches(0.15), y + Inches(0.06), Inches(5.7), Inches(0.24)),
            title,
            11,
            WHITE,
            bold=True,
    ...
    foot(
        s,
        "Lockfile discipline is deliberate: adding scipy/matplotlib to the measurement venv "
    ...
    algos = [
        (
            ...
            ACCENT_DK,
            "What it does",
            "Unscented Kalman filter on the fast state; emits the innovation "
            "vector and its Mahalanobis distance under the innovation covariance.",
            "Why chosen",
            "Standard, well-understood non-linear estimator; innovation is the "
            "natural residual for FDI at the estimation stage.",
            "Inputs / outputs",
            "Fused sensor frame → posterior mean/covariance and residual statistics.",
            "Notes",
            "The innovation is the substrate for the L3 trust index and, in this study, "
            "for the D_L2a stage-wise discriminability.",
        ),
        (
            "L4  PPO with PID-Lagrangian CMDP",
            MUTE,
            "What it does",
            "Learns a lateral-control policy under three constraint costs "
            "(comfort, envelope, actuator).",
            "Why chosen",
            "PID-Lagrangian is the standard treatment for constrained MDPs; PPO is "
            "sample-efficient and well-supported.",
            "Training",
            "Offline, on the synthetic driving environment (kinematic bicycle). "
            "Multipliers held fixed within a batch, updated between batches.",
            "Notes",
            "This is Core-A. Its output is the untrusted proposal π_prop. It is NEVER "
            "trained on real vehicle data in the current work.",
        ),
        (
            "L6  Inductive Conformal Prediction",
            ACCENT,
            "What it does",
            "Non-parametric anomaly gate: computes non-conformity score "
            "s = ‖π_prop − π_twin‖ / √P̂_a and thresholds against a calibration corpus quantile.",
            "Why chosen",
            "Distribution-free guarantee of false-alarm rate under exchangeability; "
            "no parametric assumption on the residual distribution.",
            "Inputs / outputs",
            "Proposal and twin prediction → score → PASS / VETO verdict.",
            "Notes",
            "Whether exchangeability actually holds is the whole subject of Phase 5 (E18 "
    ...
    p(
        tf,
        "AUC( T_s | faulted , T_s | matched-clean ), folded to [0.5, 1.0]. "
    ...
    p(
        tf,
        "|{t : score_t > q}| / |E_r| computed per run; unit of analysis is the run, "
        "never the tick.",
        9,
        BODY,
        space=8,
    ...
    p(
        tf,
        "At ε=0.05, band = [2.5%, 10%]. A calibration is 'valid' only if the majority of "
        "held-out clean runs fall inside this band.",
        9,
        BODY,
        space=8,
        italic=True,
    ...
    p(
        tf,
        "During-fault and post-fault alarm rates reported separately. Aggregate rates would "
        "have hidden the R3b/R3c finding.",
        9,
        BODY,
        space=0,
        italic=True,
    ...
    p(
        tf,
        "→ constructs the pipeline, drives it for N ticks, calls an observer per tick, "
    ...
    p(
        tf,
        "1. Load frozen threshold (v3, hard-coded)\n"
    ...
    p(
        tf,
        "[NEEDS INPUT] — the project has no user-facing UI; command-line drivers and "
        ...
        "figure such as fig1_stage_profile_P1.svg.",
        9,
        MUTE,
        italic=True,
        space=0,
    ...
    p(
        tf,
        "Cross-check that the self-trust / monitorability contribution is unclaimed. "
        "In progress; blocks the novelty claim.",
        9.5,
        BODY,
        space=6,
    )
    ...
    p(
        tf,
        "Compute M(f,s) on the 28 existing cells. Zero new compute, hours of work; validates "
        "or removes the paper's proposed metric.",
        9.5,
        BODY,
        space=6,
    )
    ...
    p(
        tf,
        "Unblocked on P1 after E18-R3 PASS; awaits Phase 2 result before design finalisation.",
        9.5,
        BODY,
        space=6,
    )
    ...
    p(
        tf,
        "Design complete (ASTRA 2.0 review). Implementation as production code is the "
    ...
    stages = [
        (
            "Completed",
            "5 experiment phases; 4,300 runs; audited findings; withdrawn claims "
            "documented; ASTRA 2.0 architecture proposed",
            DONE_TXT := "DONE",
            OK,
        ),
        (
            "Current stage",
            "Literature review + Phase 2 monitorability re-analysis + P3 "
            "recalibration + E19 pre-registration",
            "IN PROGRESS",
            WARN,
        ),
        (
            "Next steps",
            "Implement self-trust layer + phase-aware detector; complete E19 monitor "
            "placement study; run E20 lying-sensor extension on the per-channel injection path",
            "NEXT",
            ACCENT,
        ),
        (
            "Final implementation",
            "Integrated ASTRA 2.0 with observability plane, monitorability "
            "output, and phase-resolved decision rule",
            "PLANNED",
            MUTE,
        ),
        (
            "Testing & validation",
            "comma2k19 real-driving-data replay (open-loop); CARLA closed-"
            "loop only if time permits; baseline comparison against innovation-χ² monitor",
            "PLANNED",
            MUTE,
        ),
        (
            "Final demo",
            "Live end-to-end: inject faults into a driven scenario, show phase-"
            "resolved detection, show the self-trust plane annotating each verdict",
            "PLANNED",
            MUTE,
        ),
    ]
    ...
    for i, (title, body, tag, col) in enumerate(stages):
        ...
        # stage number
        p(
            tb(s, Inches(0.85), y + Inches(0.24), Inches(0.7), Inches(0.4)),
            f"0{i + 1}",
            22,
            col,
            bold=True,
    ...
    foot(
        s,
        "Dates are deliberately not specified. Progression is gated: a stage is not entered "
    ...
    p(
        tf,
        "All results below are on the synthetic plant with three learned policy checkpoints. "
        "Positive findings are supported by ≥ 30 seeds and pre-registered criteria. "
        "External-data validation is future work.",
        10,
        MUTE,
        first=True,
        space=0,
    ...
    table(
        ...
        [
            ...
            [
                ...
                "30/30 held-out clean runs in the [2.5 %, 10 %] band at n = 3,200 (E18-R3)",
                "Established",
            ],
            [
                "Windowed calibration limit was precision, not dynamics",
                "Per-run FAR variance scales with n; slope −0.739 (P1), −0.918 (P3) (E18-R3)",
                "Established",
            ],
            [
            ...
            [
                ...
                "clean baseline (E18); phase-resolved mechanism confirmed by R3c",
                "Established",
            ],
            [
    ...
    foot(
        s,
        "Six earlier claims were withdrawn during the campaign after measurement defects were "
    ...
    foot(
        s,
        "The withdrawal record is treated as an asset of the project: a reviewer can inspect "
    ...
    items = [
        (
            ...
            "run E19 monitor-placement experiment on P1 with pre-registered baselines.",
            ACCENT,
        ),
        (
            "Medium-term (months)",
