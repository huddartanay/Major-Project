...
| 4g-iii | Zheng, Li, Ruchkin, Piskac & Pajic, *NeuroStrata: Harnessing Neurosymbolic Paradigms for Improved Design, Testability, and Verifiability of Autonomous CPS*, FSE Companion 2025 (ACM, open access), arXiv 2502.12267v2 | PDF (all 5 pp.) | any runtime check of a monitor's detection capability, or sensor-fault handling? | **no** — a 5-page **vision paper**: hierarchical neurosymbolic framework (specification mining, design-time synthesis, runtime adaptation via program induction). Only result is LASER (spec-aligned training of scene-graph perception on video datasets). Runtime validation of neural outputs against symbolic specs is proposed, not built or evaluated; no faults, no monitor-of-monitor | §3 pp. 2–3; §4 pp. 4 | **No overlap.** Optional citation for the *semantic-errors* part of the goal: layered symbolic consistency specs (e.g. sensor-fusion accuracy within 0.1 m) are proposed as future work |
| 6a | Antonante, Nilsen & Carlone, *Monitoring of Perception Systems: Deterministic, Probabilistic, and Learning-based Fault Detection and Identification*, arXiv 2205.10906v1 (AI Journal) | PDF (§1–4, §6 definitions, §7 results summary, §8; text extraction) | is cross-layer disagreement already used to detect a monitor that cannot detect? | **closest cross-layer work found — partly yes.** Diagnostic tests = consistency checks **between modules** (e.g. LiDAR vs camera detections); a **diagnostic graph** infers which module failed from the syndrome, with κ-diagnosability limits (from the PMC multiprocessor-diagnosis model) and PAC bounds. **Crucially, tests are modelled as unreliable**: a *Weak-OR / Weaker-OR* test may **PASS when its whole scope has failed** because common-mode failures stay mutually consistent (two detectors missing the same obstacle). Evaluated in LGSVL + Apollo; perception modules only — **localization / state estimation listed as future work**; reports missed detections when all detectors fail consistently | §4.1 pp. 14–16; Def. 7 p. 15; Ex. 3 pp. 24–25; §6 p. 33+; Fig. 18 p. 60; §8 p. 61 | **Narrows B1's cross-layer form.** Cross-module consistency checks and a formal model of tests that stay silent under common-mode failure exist. What remains for ASTRA: (i) the **safety gate itself** is a node to be diagnosed — Antonante's tests are monitors whose unreliability is *assumed a priori*, never *detected at runtime*; (ii) state estimation / sensor faults (their future work); (iii) closed-loop persistent faults. Must be cited as the framework ASTRA extends; consider phrasing B1 as a diagnostic test whose scope includes the gate |
| 6b | Orf, Ochs, Doll, Schotschneider, Heinrich, Zofka & Zöllner, *Modular Fault Diagnosis Framework for Complex Autonomous Driving Systems*, arXiv 2411.09643v1 | PDF (all 8 pp.) | is disagreement between modules used as fault evidence? | **as a category only.** Taxonomy includes "contextual, parallel data flow" diagnosis on redundant data; implemented diagnoses are mostly message frequency/latency and one localization-quality module. Aggregation is OR over module states plus a dependency graph that marks downstream groups IGNORE. Six qualitative scenarios on shuttles; no detection rates. States that **self-diagnosis may be overconfident and suffer from the same faults as the component itself** | §IV-B p. 3; §IV-D p. 4; §V–VI pp. 5–7 | **No overlap** with B1. Citable sentence on self-diagnosis sharing the component's faults (supports shared-path framing). Note: its dependency IGNORE *suppresses* downstream diagnoses — the opposite of flagging a silent monitor |
| 6c | Kempa, Zhang, Jones, Zambreno & Rozier, *Embedding Online Runtime Verification for Fault Disambiguation on Robonaut2*, FORMATS 2020 | PDF (§1, §5, §6 read; FPGA algorithm sections skimmed) | does runtime verification separate sensor faults from system faults? | **yes, by classic sensor voting.** MLTL specs on FPGA compare two position sensors and an encoder on one knee joint; the sensor disagreeing with the others is declared faulty; faults must persist 3 steps. Validated on recorded traces. Revealed a **hidden assumption** (encoder freezes during a fault) only after deployment data | §5.2 pp. 12–15; §6 p. 16 | **No overlap** with B1 (monitors sensors, not a monitor). Supports N5 wording: redundancy voting among sensors is classical — ASTRA's disagreement is between **layers of the safety stack**, not between redundant sensors. Anecdote supports "monitor assumptions fail silently in the field" |
| 6d | Harder, Kulkarni & Behl, *HALO*, arXiv 2503.10341v1 — §10 re-check | PDF (grep of full text) | any mechanism acting on disagreement between health and behaviour monitors? | **no** — node heartbeats (incl. a diagnostics heartbeat) trigger graceful stop; voting planes rejected for hardware reasons. Liveness only, as recorded earlier | pp. 3, 10–11 | Confirms A3 "liveness" row |
