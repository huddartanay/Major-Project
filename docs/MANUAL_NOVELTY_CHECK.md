...
| 4n | Peper et al., *Towards Unified Probabilistic Verification and Validation of Vision-Based Autonomy*, ATVA 2025, arXiv 2508.14181 | | does the guarantee **detect or adapt** when test-time conditions leave the calibrated distribution? | *to check* — abstract says the guarantee adapts to out-of-distribution test-time conditions, and that prior guarantees rest on rigid assumptions | | **Highest remaining risk** to "the guarantee lapses silently"; the abstract's framing also supports the gap |
...
| 4p | Mao et al., *How Safe Will I Be Given What I Saw?*, arXiv 2508.09346 (2025) | | is miscalibration under shift **detected** at runtime, and is shift caused by sensor faults? | *to check* — abstract: domain adaptation keeps safety evaluators calibrated under shift in predictions | | Could narrow B1 if it detects evaluator miscalibration online |
| … | | | | | | |
