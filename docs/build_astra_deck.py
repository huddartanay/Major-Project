...
def slide(prs, title=None, kicker=None):
    ...
    if title:
        if kicker:
            _p(
                _tb(s, Inches(0.7), Inches(0.42), Inches(11.9), Inches(0.24)),
                kicker.upper(),
                10,
                ACCENT,
                bold=True,
        ...
        _p(
            _tb(s, Inches(0.7), Inches(0.68), Inches(11.9), Inches(0.6)),
            title,
            27,
            INK,
            bold=True,
...
def foot(s, text):
    _p(
        _tb(s, Inches(0.7), Inches(6.92), Inches(11.9), Inches(0.3)),
        text,
        8.5,
        FAINT,
        first=True,
    ...
    for i, label in enumerate(items):
        ...
        if i < len(items) - 1:
            if vertical:
                ...
                _p(
                    _tb(slide, bx + w, by + Inches(0.14), gap, Inches(0.3)),
                    "▶",
                    9,
                    FAINT,
                    first=True,
...
def build(out: Path, pos: dict | None) -> None:
    ...
    _p(
        _tb(s, Inches(0.9), Inches(1.55), Inches(11.5), Inches(0.4)),
        "RESEARCH & COLLABORATION BRIEFING",
        12,
        RGBColor(0x7E, 0xA8, 0xC9),
        bold=True,
        first=True,
        space=0,
    )
    ...
    _p(
        tf,
        "Fault Observability, Information Loss & Monitor Placement",
        23,
        RGBColor(0xDA, 0xE4, 0xEE),
        space=2,
    )
    _p(tf, "in Autonomous Driving Systems", 23, RGBColor(0xDA, 0xE4, 0xEE), space=14)
    _p(
        tf,
        "Current Evidence  •  Research Roadmap  •  Dataset Validation  •  "
        "Industry Value  •  Publication Potential",
        13,
        RGBColor(0x9F, 0xBA, 0xD2),
    )
    _p(
        _tb(s, Inches(0.9), Inches(6.35), Inches(11.5), Inches(0.6)),
        "Sushanth C.  ·  Tarun Gowda V  ·  T Tilak Reddy   |   Guide: Dr. Chaitra R.   |   "
        "BMS College of Engineering   |   1 September 2026",
        11,
        RGBColor(0x8D, 0xA5, 0xBC),
        first=True,
        space=0,
    )
    ...
    kv_panel(
        ...
        [
            ...
            "Three measurement defects found and corrected — two by self-audit",
        ],
        EST,
    )
    kv_panel(
        ...
        [
            ...
            "Three other faults behave in three other ways",
        ],
        PART,
    )
    kv_panel(
        ...
        [
            ...
            "Adversarial extension not started",
        ],
        PLAN,
    )
    ...
    _p(
        tf,
        "ASTRA has a technically audited foundation and a scientifically interesting "
        ...
        "adversarial conditions and closed-loop driving.",
        12,
        MUTED,
        space=0,
    )
    foot(
        s,
        "Every claim in this deck is badged ESTABLISHED / PARTIALLY VALIDATED / PLANNED / "
    ...
    _p(tf, "A sensor fault does not stay a sensor fault.", 15, INK, bold=True, first=True, space=10)
    _p(
        tf,
        "It passes through estimation, fusion, trust scoring and a stack of safety "
    ...
    labels = [
        ("L1  sensing", "evidence strong", EST),
    ...
    for name, note, col in labels:
        ...
        _p(
            _tb(s, Inches(9.55), yy + Inches(0.1), Inches(2.8), Inches(0.3)),
            note,
            10.5,
            MUTED,
            first=True,
        ...
        yy += Inches(0.55)
    _p(
        _tb(s, Inches(7.05), Inches(4.72), Inches(5.3), Inches(0.5)),
        "The monitor sits after the point where the evidence became insufficient — "
        "so it cannot act, however well it is implemented.",
        11,
        INK,
        first=True,
        space=0,
    ...
    foot(
        s,
        "Illustrative schematic of the problem class. Measured per-fault profiles appear "
    ...
    _p(
        tf,
        "A framework for quantifying how sensor-fault evidence changes across an "
        ...
        "investigating how that guides monitor placement.",
        13,
        INK,
        bold=True,
        first=True,
    ...
    _p(
        tf,
        "When a sensor becomes faulty, where in the pipeline does the fault stop being "
        "distinguishable from healthy behaviour?",
        13,
        INK,
        bold=True,
        space=12,
    ...
    _p(
        tf,
        "Can that information decide where monitoring should occur?",
        13,
        INK,
        bold=True,
        space=12,
    ...
    kv_panel(
        ...
        [
            ...
            ("transformation — not whether a fault is present.", INK, True),
        ],
        MUTED,
    )
    ...
    _p(
        _tb(s, Inches(0.7), Inches(1.5), Inches(11.9), Inches(0.3)),
        "Measurement points used by the experiment are marked. Core-A proposes; Core-B may "
        "veto. The trust boundary is one-way.",
        11,
        MUTED,
        first=True,
        space=0,
    ...
    for code, name, col in cells:
        _rect(s, x, Inches(2.1), bw, Inches(0.95), fill=col, shape=MSO_SHAPE.ROUNDED_RECTANGLE)
        t2 = s.shapes[-1].text_frame
        t2.word_wrap = True
    ...
    _p(
        _tb(s, Inches(0.7), Inches(3.24), Inches(4.72), Inches(0.25)),
        "SHARED  ·  L1–L3",
        9.5,
        MUTED,
        first=True,
    ...
    _p(
        _tb(s, Inches(5.5), Inches(3.24), Inches(1.13), Inches(0.25)),
        "CORE-A",
        9.5,
        MUTED,
        first=True,
    ...
    _p(
        _tb(s, Inches(6.71), Inches(3.24), Inches(4.68), Inches(0.25)),
        "CORE-B  ·  3 gates verdict on every tick",
        9.5,
        MUTED,
        first=True,
        space=0,
    ...
    _p(
        _tb(s, Inches(2.7), Inches(3.76), Inches(9.9), Inches(0.32)),
        "9 / 9 layers domain-mapped · 3 / 3 Core-B gates produce a verdict on every one of "
        "300 ticks — verified in a live run, not from documentation.",
        11.5,
        INK,
        first=True,
        space=0,
    ...
    foot(
        s,
        "Stage statistics are one defensible choice among several; a different statistic could "
    ...
    _p(
        _tb(s, Inches(4.45), Inches(1.62), Inches(8.15), Inches(0.7)),
    ...
    kv_panel(
        ...
        [
            ...
            ("0 of 12 cells. The original D = 0.500 was entirely the artefact.", INK, True),
        ],
        PART,
    )
    kv_panel(
        ...
        [
            ...
            ("delivered-signal verification before any result is accepted.", MUTED),
        ],
        EST,
    )
    ...
    _p(
        _tb(s, Inches(0.7), Inches(1.55), Inches(11.9), Inches(0.3)),
    ...
    _p(
        tf,
        "0.5 is chance-level discriminability and the floor of a folded metric. It means "
        "the evaluated representation does not retain discriminative evidence — not that "
        "a detector is right half the time.",
        11,
        MUTED,
        space=0,
    )
    ...
    _p(
        tf,
        "AUC is scale-free. It reports near-perfect separation for a shift far below a "
    ...
    _p(
        _tb(s, Inches(0.7), Inches(3.52), Inches(11.9), Inches(0.3)),
    ...
    table(
        ...
        [
            ...
            [
                "speed_stuck",
                "Absorbed at estimation",
                "L2a — well-posed 30/30 (P1)",
                "0.9625 → 0.5629, SD 0.0077",
            ],
            [
                "speed_bias",
                "Weakens, then statistic recovers",
                "not well-posed — 0/30 seeds",
            ...
            [
                "position_bias / drift",
                "Survives estimation (corrected)",
                "none — not absorbed",
                "D_L2a 0.655–0.797, n=30",
            ],
        ],
        widths=[1.7, 2.9, 3.3, 3.4],
        fs=10.5,
    )
    _badge(s, Inches(0.7), Inches(6.46), "PARTIALLY VALIDATED")
    _p(
        _tb(s, Inches(2.7), Inches(6.44), Inches(9.9), Inches(0.32)),
        "Heterogeneity is the finding: a single absorption point is ill-posed for five of six "
        "faults. One fault-policy pair shows a clean, well-posed absorption.",
        11.5,
        INK,
        first=True,
        space=0,
    ...
    foot(
        s,
        "One plant model · one severity per fault · three policies · simulation only. "
    ...
    for t in [
        "FaultChannel.POSITION_Y is inert against the driven sensing path under "
    ...
    for t in [
        "“Position faults are absorbed at the estimator.” — the faults never reached the "
    ...
    foot(
        s,
        "Three measurement defects were found in this project's own code, two of them by "
    ...
    _p(
        tf,
        "Sensor-fault evidence degrades non-uniformly across pipeline stages, and the "
        "stage at which it becomes insufficient is a measurable, fault-specific property "
        "that can inform where monitors are placed.",
        14,
        INK,
        bold=True,
        first=True,
    ...
    _p(
        _tb(s, Inches(0.7), Inches(3.0), Inches(11.9), Inches(0.3)),
    ...
    _p(
        tf,
        "Can the fault be recognised from the raw corrupted measurement?     |     "
        "Does it remain distinguishable after state estimation?     |     "
        "Can a real gate, with its real threshold, act on it?",
        11,
        MUTED,
        first=True,
        space=0,
    ...
    _p(
        ...
        "proposed contribution is the combination.",
        12,
        MUTED,
        first=True,
        space=0,
    ...
    table(
        ...
        [
            ...
            [
                "1",
                "Stage-wise fault discriminability across a governance pipeline",
                "Measured on 4 valid faults",
            ],
            [
                "2",
                "Fault-specific observability trajectories",
                "Measured — four distinct behaviours",
            ],
            ["3", "A candidate fault-absorption point A(f)", "Ill-posed for 5 of 6 faults"],
            [
                "4",
                "Separating statistical discriminability from operational gate detection",
                "Measured — and consequential",
            ],
            [
                "5",
                "Using measured observability to investigate monitor placement",
                "PLANNED — experiment H7",
            ],
    ...
    _p(
        _tb(s, Inches(1.0), Inches(5.52), Inches(11.3), Inches(0.6)),
    ...
    _p(
        tf,
        "If a fault is distinguishable at one stage but not at a later stage, does "
        "monitoring at the earlier stage provide better operational detection?",
        13,
        INK,
        bold=True,
        space=10,
    ...
    _p(
        tf,
        "This is where measured observability would become an engineering recommendation "
    ...
    kv_panel(
        ...
        [
            ...
            ("conformal quantile ≈ 5.40, a gap of ≈ 2.0.", MUTED),
        ],
        PART,
    )

    _p(
        _tb(s, Inches(0.7), Inches(4.42), Inches(11.9), Inches(0.3)),
        "H7 must therefore resolve, in order:",
        12.5,
        INK,
        bold=True,
        first=True,
    ...
    chain(
        ...
        h=Inches(0.8),
        gap=Inches(0.15),
        col=[RED, PART, PLAN, PLAN],
        fs=11,
    )
    _p(
        _tb(s, Inches(0.7), Inches(5.92), Inches(11.9), Inches(0.5)),
        "Because the first step is currently unresolved, H7 is gated behind the calibration "
        "question (OD-8). H7 has not succeeded and has not been run.",
        11.5,
        MUTED,
        first=True,
        space=0,
    ...
    table(
        ...
        [
            ...
            [
                ...
                "does not natively contain the required sensor faults",
                "PLANNED",
            ],
            [
                "highD",
                "Naturalistic traffic / regime validation",
                "trajectories, speed distributions, density, following & lane-change behaviour",
                "not a sensor-fault dataset",
                "PLANNED",
            ],
            [
                "CARLA",
                "Closed-loop validation & demonstration",
                "faults, adversarial sensors, estimator behaviour, closed-loop consequences",
                "must not be the sole source of evidence",
                "PLANNED",
            ],
        ],
        widths=[1.35, 2.5, 4.3, 2.6, 1.15],
        fs=10,
        header_fs=10,
        row_h=Inches(0.95),
    )
    kv_panel(
        ...
        [
            (
                ...
                "are verified before a dataset is treated as a confirmed resource.",
                MUTED,
            )
        ],
        ACCENT,
    ...
    kv_panel(
        ...
        [
            (
                ...
                "pipeline is driven by real vehicle data?",
                INK,
                True,
            ),
            ...
            (
                ...
                "injection over real measurements is the method.",
                MUTED,
            ),
            (
                "Status: conditional — verification of timestamp precision, storage, "
                "calibration and drive identity is outstanding.",
                PART,
                True,
            ),
        ],
    ...
    _p(
        tf,
        "Additional dataset opportunities — only if scientifically justified",
        12.5,
        INK,
        bold=True,
        first=True,
    ...
    _p(
        tf,
        "Categories under consideration: autonomous-driving datasets, localisation/sensor "
    ...
    _p(
        _tb(s, Inches(0.7), Inches(5.78), Inches(11.9), Inches(0.6)),
    ...
    _badge(s, Inches(0.7), Inches(1.58), "PLANNED", w=Inches(1.3))
    _p(
        _tb(s, Inches(2.2), Inches(1.56), Inches(10.4), Inches(0.32)),
    ...
    for label, col, note in [
        ...
        _rect(
            s, Inches(0.7), y, Inches(3.5), Inches(0.5), fill=col, shape=MSO_SHAPE.ROUNDED_RECTANGLE
        )
        t2 = s.shapes[-1].text_frame
        t2.margin_left = Inches(0.12)
        _p(t2, label, 11.5, WHITE, bold=True, first=True, space=0)
        _p(
            _tb(s, Inches(4.35), y + Inches(0.13), Inches(2.2), Inches(0.3)),
            note,
            10.5,
            MUTED,
            first=True,
    ...
    chain(
        ...
        [
            ...
            "ASTRA stage-wise observability",
    ...
    kv_panel(
        ...
        [
            (
                ...
                "remain observable, or become hidden by sensor fusion?",
                INK,
                True,
            ),
            (
    ...
    steps = [
        ("E17 audit", EST),
        ("Fix position injection", PART),
        ("Correct L6 interpretation", PART),
        ("Confirm speed_stuck metric", PART),
        ("Corrected E17 validation", PART),
        ("H7 monitor placement", PLAN),
        ("comma2k19 real data", PLAN),
        ("highD regime validation", PLAN),
        ("Adversarial sensors", PLAN),
        ("CARLA closed loop", PLAN),
        ("Integrated evaluation", HYP),
        ("Conference paper", HYP),
    ...
    for i, (label, col) in enumerate(steps):
        ...
        if c < 3 and i < len(steps) - 1:
            _p(
                _tb(s, bx + bw2, by + Inches(0.16), Inches(0.18), Inches(0.3)),
                "▶",
                9,
                FAINT,
                first=True,
    ...
    _p(
        tf,
        "A stage is not entered because it exists in the roadmap. Each must answer a "
        ...
        "presented as a source of primary evidence.",
        11.5,
        MUTED,
        space=0,
    )
    ...
    _p(
        _tb(s, Inches(1.0), Inches(5.94), Inches(11.3), Inches(0.6)),
    ...
    _p(
        _tb(s, Inches(0.7), Inches(1.55), Inches(5.85), Inches(0.3)),
        "TODAY — binary detection",
        11,
        MUTED,
        bold=True,
        first=True,
    ...
    chain(
        ...
        Inches(0.7),
        Inches(1.92),
        ["Fault occurs", "Detector: “fault / no fault”"],
        w=Inches(2.84),
        h=Inches(0.55),
        gap=Inches(0.15),
        col=[MUTED, MUTED],
    ...
    _p(
        _tb(s, Inches(0.7), Inches(2.62), Inches(5.85), Inches(0.4)),
        "Limited information about where the fault became invisible.",
        11,
        MUTED,
        first=True,
        space=0,
    ...
    _p(
        _tb(s, Inches(6.75), Inches(1.55), Inches(5.85), Inches(0.3)),
        "POTENTIAL WITH ASTRA — stage-wise profile",
        11,
        ACCENT,
        bold=True,
        first=True,
    ...
    prof = [
        ("L1  sensing", "visible", EST),
        ("L2  estimation", "visible", EST),
        ("L2a innovation", "weak", PART),
        ("L7  shield", "insufficient", RED),
    ]
    yy = Inches(1.92)
    for name, note, col in prof:
        ...
        _p(
            _tb(s, Inches(9.4), yy + Inches(0.06), Inches(3.2), Inches(0.28)),
            note,
            10,
            MUTED,
            first=True,
        ...
        yy += Inches(0.4)
    _p(
        _tb(s, Inches(6.75), Inches(3.55), Inches(5.85), Inches(0.3)),
        "→ “the architecture has a monitoring blind spot here”",
        11.5,
        INK,
        bold=True,
        first=True,
    ...
    _p(
        _tb(s, Inches(0.7), Inches(4.05), Inches(11.9), Inches(0.3)),
        "Possible future output format",
        12,
        INK,
        bold=True,
        first=True,
    ...
    _p(
        _tb(s, Inches(0.95), Inches(4.55), Inches(11.5), Inches(0.3)),
    ...
    _p(
        _tb(s, Inches(0.7), Inches(5.15), Inches(11.9), Inches(0.3)),
    ...
    for t in [
        "validation on company-specific sensor architectures",
    ...
    for t in [
        "adversarial sensor testing (once validated)",
    ...
    _p(
        _tb(s, Inches(0.7), Inches(6.35), Inches(11.9), Inches(0.4)),
    ...
    tiers = [
        ...
        (
            ...
            [
                ...
                "monitor-placement consequence",
                "adversarial extension",
                "closed-loop CARLA validation",
    ...
    for i, (name, col, reqs, note) in enumerate(tiers):
        ...
        _p(
            _tb(s, bx + Inches(0.2), Inches(5.35), Inches(3.5), Inches(0.75)),
            note,
            10.5,
            INK,
            bold=True,
    ...
    _p(
        _tb(s, Inches(1.0), Inches(6.44), Inches(11.4), Inches(0.3)),
        "No acceptance probability can be guaranteed. The objective is to make the scientific "
        "contribution strong enough for a credible submission.",
        11.5,
        WHITE,
        bold=True,
        first=True,
    ...
    _badge(s, Inches(0.7), Inches(1.6), "ESTABLISHED", w=Inches(1.6))
    _p(
        _tb(s, Inches(0.7), Inches(1.98), Inches(5.85), Inches(0.3)),
        "What we can officially claim today",
        12.5,
        INK,
        bold=True,
        first=True,
    ...
    for t in [
        "POSITION_Y is inert under ADR-0033 — causally demonstrated (Control C)",
    ...
    _p(
        _tb(s, Inches(6.75), Inches(1.98), Inches(5.85), Inches(0.3)),
    ...
    _rect(s, 0, 0, W, H, fill=INK)
    _p(
        _tb(s, Inches(0.9), Inches(0.85), Inches(11.5), Inches(0.3)),
    ...
    _p(
        tf,
        "The current E17 result is not the final product. It is the controlled foundation "
    ...
    _p(
        tf,
        "ASTRA already has a technically audited foundation and a scientifically "
        ...
        13,
        RGBColor(0xC2, 0xD3, 0xE2),
        space=0,
    )
    _p(
        _tb(s, Inches(0.9), Inches(6.55), Inches(11.5), Inches(0.4)),
...
if __name__ == "__main__":
    ...
    build(
        root / "docs" / "ASTRA_Research_Collaboration_Briefing.pptx",
