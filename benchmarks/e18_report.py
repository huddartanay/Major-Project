...
def main() -> None:
    ...
    args = ap.parse_args()
    rows = json.loads(
        (args.dir / "raw_results" / "fault_evaluation.json").read_text(encoding="utf-8")
    )
    ...
    if bad:
        ...
        for r in bad:
            grp[(r["policy"], r["fault"], r["severity_level"], r["severity_value"])].append(
                r["seed"]
            )
    ...
    print(
        "OPERATIONAL DETECTION at the frozen threshold  (P2 shown but EXCLUDED - drift criterion)"
    )
    print(
        f"{'pol':<4}{'fault':<16}{'sev':<8}{'value':>8}{'det.rate':>10}{'alarm rate':>12}"
        ...
        for fault, spec in SEVERITIES.items():
            for lvl in sorted(spec["levels"], key=lambda k: LEVEL_ORDER.get(k, 9)):
                g = [
                    r
                    for r in ok
                    if r["policy"] == p and r["fault"] == fault and r["severity_level"] == lvl
                ]
                if not g:
                    continue
                ...
                lat = [
                    r["detection_latency_ticks"]
                    for r in g
                    if r["detection_latency_ticks"] is not None
                ]
                ...
                detect[f"{p}|{fault}|{lvl}"] = {
                    "detection_rate": det,
                    "detection_ci": [ci["lo"], ci["hi"]],
                    "alarm_rate_faulted": alarm,
                    "clean_far": CLEAN_FAR[p],
                    "median_latency_ticks": float(np.median(lat)) if lat else None,
                    "threshold_margin": margin,
                    "D": {
                        s: float(np.nanmedian([r["D"].get(s, np.nan) for r in g])) for s in STAGES
                    },
                    "severity_value": g[0]["severity_value"],
                    "n": len(g),
                    "policy_valid": p in VALID_POLICIES,
                }
                print(
                    f"{p:<4}{fault:<16}{lvl:<8}"
    ...
    for p in VALID_POLICIES:
        for fault, spec in SEVERITIES.items():
            ...
            label = (
                "NOT DETECTED at any tested severity"
                if found is None
                else f"{found[0]} ({found[1] if found[1] is not None else 'n/a'} {spec['unit']})"
            )
        ...
        for key, v in detect.items():
            ...
            w.writerow(
                [
                    ...
                    v["severity_value"],
                    v["n"],
                    f"{v['detection_rate']:.4f}",
                    f"{v['detection_ci'][0]:.4f}",
                    f"{v['detection_ci'][1]:.4f}",
                    f"{v['alarm_rate_faulted']:.4f}",
                    f"{v['clean_far']:.4f}",
                    "" if v["median_latency_ticks"] is None else f"{v['median_latency_ticks']:.0f}",
                    f"{v['threshold_margin']:.4f}",
                    FROZEN_QUANTILE[p],
                    *[f"{v['D'][s]:.4f}" for s in STAGES],
                ]
            )
    (proc / "detection_analysis.json").write_text(
