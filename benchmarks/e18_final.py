...
def axes(xlo, xhi, ylo, yhi, xlab, ylab, w=W, h=H, pl=62, pr=18, pt=34, pb=44):
    ...
    def X(v):
        return x0 + (x1 - x0) * (v - xlo) / (xhi - xlo) if xhi > xlo else x0

    def Y(v):
        return y0 + (y1 - y0) * (v - ylo) / (yhi - ylo) if yhi > ylo else y0
    ...
    for k in range(6):
        ...
        p.append(
            f'<text x="{x0 - 6}" y="{Y(v) + 3:.1f}" text-anchor="end" fill="#444">{v:.3g}</text>'
        )
    for k in range(6):
        v = xlo + (xhi - xlo) * k / 5
        p.append(
            f'<text x="{X(v):.1f}" y="{y0 + 15}" text-anchor="middle" fill="#444">{v:.3g}</text>'
        )
    p.append(
        f'<text x="{(x0 + x1) / 2:.0f}" y="{h - 8}" text-anchor="middle" fill="#333">{xlab}</text>'
    )
    p.append(
        f'<text x="14" y="{(y0 + y1) / 2:.0f}" text-anchor="middle" fill="#333" '
...
def main() -> None:
    ...
    def runs(doc, p):
        return [np.array(r["scores"], float) for r in doc["runs"][p]]

    def flat(doc, p):
        x = np.concatenate(runs(doc, p))
        return x[np.isfinite(x)]

    # ---- Table A + per-run FAR (the diagnostic pooled FAR hides) ----------
    print("TABLE A - CALIBRATION")
    print(
        f"{'pol':<5}{'cal N':>8}{'threshold':>11}{'eps':>7}{'exp FAR':>9}{'held-out FAR':>14}"
    ...
    for p in POLICIES:
        ...
        pooled = (
            float(np.concatenate([r[np.isfinite(r)] for r in runs(tst, p)]) > q).__float__()
            if False
            else float((np.concatenate([r[np.isfinite(r)] for r in runs(tst, p)]) > q).mean())
        )
        inband = int(((per >= EPS / 2) & (per <= 2 * EPS)).sum())
        ci = bca_median_ci(per)
        tableA[p] = {
            "calibration_n": int(c.size),
            "threshold": q,
            "epsilon": EPS,
            "expected_far": EPS,
            "heldout_far_pooled": pooled,
            "per_run_far_median": ci["median"],
            "per_run_far_ci": [ci["lo"], ci["hi"]],
        ...
        print(
            f"{p:<5}{c.size:>8}{q:>11.4f}{EPS:>7.2f}{EPS:>9.1%}{pooled:>14.2%}"
    ...
    print(
        f"{'pol':<5}{'1st half':>10}{'2nd half':>10}{'drift':>9}{'drift/SD':>10}"
    ...
    for p in POLICIES:
        ...
        stat[p] = {
            "first_half": float(h1.mean()),
            "second_half": float(h2.mean()),
            "drift": float(h2.mean() - h1.mean()),
            "drift_over_sd": float(abs(h2.mean() - h1.mean()) / sd),
            "runs_with_positive_drift": int((h2 > h1).sum()),
            "n_runs": len(rs),
            "wilcoxon_p": w["p"],
        }
        print(
            f"{p:<5}{h1.mean():>10.4f}{h2.mean():>10.4f}{h2.mean() - h1.mean():>9.4f}"
    ...
    print(
        f"{'pol':<5}{'fault':<16}{'sev':<8}{'fault alarm':>13}{'clean FAR':>11}{'ratio':>8}"
                ...
                supp.append(
                    {
                        ...
                        "clean_far": float(clean_per.mean()),
                        "ratio": float(ratio),
                        "wilcoxon_p": w["p"],
                        "suppressed": is_supp,
    ...
    for r in dis[:8]:
        print(
            f"    {r['policy']} {r['fault']:<15} {r['severity']:<7} "
    ...
    for p in POLICIES:
        ...
        thr = (
            f'<line x1="{X(q):.1f}" y1="{Y(0):.1f}" x2="{X(q):.1f}" y2="{Y(1.05):.1f}" '
        ...
        (fig / f"fig1_clean_distribution_{p}.svg").write_text(
            svg(
                frag + bars + thr,
                f"Figure 1{p} - clean calibration scores and frozen threshold ({p}, n={c.size})",
            ),
            encoding="utf-8",
        )
    ...
    for p in POLICIES:
        ...
        for arr, col, off in ((c, "#1b6ca8", 0), (t, "#b8860b", 1)):
            ...
            body += (
                f'<rect x="{W - 190}" y="{36 + off * 14}" width="10" height="10" fill="{col}" '
    ...
    for p in POLICIES:
        rs = [r[np.isfinite(r)] for r in runs(cal, p)]
        lo = min(r.min() for r in rs)
        hi = max(r.max() for r in rs)
        ...
        if lo <= q <= hi:
            body += (
                f'<line x1="{X(0):.1f}" y1="{Y(q):.1f}" x2="{X(400):.1f}" y2="{Y(q):.1f}" '
                f'stroke="#b0362a" stroke-dasharray="5,3"/>'
            )
        d = stat[p]
        body += (
            f'<text x="{X(10):.1f}" y="{Y(hi) - 6:.1f}" fill="#333">'
        ...
        (fig / f"fig6_temporal_drift_{p}.svg").write_text(
            svg(body, f"Figure 6{p} - within-run score trajectories ({p}, 12 of 30 runs)"),
            encoding="utf-8",
        )

    # F7: D_L6 vs operational detection
    frag, X, Y = axes(
        0.4, 1.02, -0.05, 1.05, "D_L6  (statistical discriminability)", "operational detection rate"
    )
    body = frag
    for r in tc:
        col = PAL[r["policy"]]
        body += (
            f'<circle cx="{X(min(r["D_L6"], 1.02)):.1f}" cy="{Y(r["detection_rate"]):.1f}" '
            f'r="3.6" fill="{col}" opacity="0.75"/>'
        )
    body += (
        f'<text x="{X(0.42):.1f}" y="{Y(1.0):.1f}" fill="#333">'
        f"Spearman rho = {sp['rho']:+.3f}, p = {sp['p']:.3f}   "
        f"{len(dis)}/{len(tc)} cells disagree</text>"
    )
    for i, p in enumerate(("P1", "P3")):
        body += (
            f'<rect x="{W - 90}" y="{36 + i * 14}" width="10" height="10" fill="{PAL[p]}"/>'
    ...
    body += (
        f'<line x1="{X(-0.4):.1f}" y1="{Y(0.9):.1f}" x2="{X(2.4):.1f}" y2="{Y(0.9):.1f}" '
        ...
        for p, v in tableA.items():
            w.writerow(
                [
                    ...
                    v["epsilon"],
                    f"{v['expected_far']:.4f}",
                    f"{v['heldout_far_pooled']:.4f}",
                    f"{v['per_run_far_median']:.4f}",
                    f"{v['per_run_far_ci'][0]:.4f}",
                    f"{v['per_run_far_ci'][1]:.4f}",
                    v["runs_in_band"],
                    v["n_runs"],
                    v["status"],
                ]
    ...
    with (proc / "alarm_suppression.csv").open("w", newline="", encoding="utf-8") as fh:
        ...
        w.writeheader()
        w.writerows(supp)
    (proc / "final_analysis.json").write_text(
