...
def main() -> None:
    ...
    manifest = json.loads(
        (args.dir / "manifests" / "run_manifest.json").read_text(encoding="utf-8")
    )
    ...
    with (args.out / "E17_ABSORPTION_POINTS.csv").open("w", newline="", encoding="utf-8") as fh:
        ...
        for p in sorted({r["policy"] for r in rows}):
            for f in FAULTS:
                ...
                counts: dict[str, int] = {}
                for v in vals:
                    counts[str(v["absorption_point"])] = (
                        counts.get(str(v["absorption_point"]), 0) + 1
                    )
    ...
    for name in (
        "E17_STAGEWISE_RESULTS",
        "E17_SEED_SUMMARY",
        "E17_RESULTS_FINAL",
        "E17_ABSORPTION_POINTS",
    ):
