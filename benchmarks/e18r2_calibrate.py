...
def main() -> None:
    ...
    for p in POLICIES:
        ...
        print(
            f"  {p}: calibration n = {c.size:>5}  nonfinite = {nonfinite}  "
    ...
    for p in POLICIES:
        ...
        for i, r in enumerate(runs_of(tst, p)):
            ...
            records.append(
                {
                    "experiment_id": "E18-R2",
                    "git_commit": commit,
                    "config_hash": config_hash,
                    "calibration_version": CALIBRATION_VERSION,
                    "policy": p,
                    "seed": tst["runs"][p][i]["seed"],
                    "condition": "clean",
                    ...
                    "alarm_count": alarms,
                    "run_far": far,
                    "in_band": bool(a.eps / 2 <= far <= 2 * a.eps),
        ...
        summary[p] = {
            ...
            "runs_in_band": inb,
            "n_runs": int(f.size),
            "pooled_far": float(f.mean()),
            "median_far": float(np.median(f)),
            "iqr": [float(np.percentile(f, 25)), float(np.percentile(f, 75))],
            "zero_alarm_runs": int((f == 0).sum()),
        }
        print(
            f"  {p}: runs in band {inb:>2}/30   median FAR {np.median(f):>6.2%}   "
    ...
    (OUT / "processed_results" / "verdict.json").write_text(
        json.dumps(
            {
                "verdict": verdict,
                "primary_runs_in_band": n,
                "P3_runs_in_band": p3,
                "P3_classification": p3class,
                "config_hash": config_hash,
